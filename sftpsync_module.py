...
def _get_plugin_path():
    ...
    plugin = os.path.join(plugin,"sftpsync")
    return plugin
...
class PSCPCommand:
    ...
    def execUpload(self,filepath):
        ...
        #    self.relative_path_ = '.'
        #print self.relative_path_
        tofile = '%s:%s/%s' % (self.pscp_cfg_.svrip,self.pscp_cfg_.svrbasepath,self.relative_path_)
    ...
    def _get_pscp_command_path(self):
        pscp = os.path.join(sublime.packages_path(), 'sftpsync')
        if sublime.arch() == "x32":
